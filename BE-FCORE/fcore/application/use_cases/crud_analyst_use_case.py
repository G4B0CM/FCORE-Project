...
class CrudAnalystUseCase:
    """Use case for CRUD operations on an Analyst."""
    ...
    def create(self, analyst_schema: AnalystCreate, created_by_code: str) -> Analyst:
        """Orchestrates the creation of a new analyst."""
        if self._analyst_repository.find_by_code(analyst_schema.code):
            raise AnalystAlreadyExistsError(f"Analyst with code {analyst_schema.code} already exists.")

        role_entity = self._role_repository.find_by_id(analyst_schema.role_id)
        ...
        new_analyst = Analyst(
            ...
            role=role_entity,
            created_at=datetime.utcnow(), # Use UTC for server time
            created_by=created_by_code
        )
        return self._analyst_repository.create(new_analyst)

    def get_all(self) -> List[Analyst]:
        """Gets all analysts."""
        return self._analyst_repository.get_all()

    def get_by_code(self, code: str) -> Analyst:
        """Gets a single analyst by their code."""
        analyst = self._analyst_repository.find_by_code(code=code)
        if not analyst:
            raise AnalystNotFoundError(f"Analyst with code {code} not found.")
        return analyst

    def get_by_id(self, id: UUID) -> Analyst:
        """Gets a single analyst by their UUID."""
        analyst = self._analyst_repository.find_by_id(id=id)
        if not analyst:
    ...
    def update(self, code: str, analyst_schema: AnalystUpdate, modified_by_code: str) -> Analyst:
        """Orchestrates the update of an existing analyst."""
        analyst_to_update = self.get_by_code(code)

        # Update fields from the schema if they were provided
        ...
        if analyst_schema.role_id is not None:
            new_role = self._role_repository.find_by_id(analyst_schema.role_id)
        ...
        # Re-validate the entity state
        analyst_to_update.__post_init__()

        return self._analyst_repository.update(analyst_to_update)
    
    def deactivate(self, code: str, modified_by_code: str) -> Analyst:
        """Deactivates an analyst (soft delete)."""
        analyst = self.get_by_code(code)
        
        analyst.deactivate()
        analyst.updated_at = datetime.utcnow()
        analyst.updated_by = modified_by_code
        
        return self._analyst_repository.update(analyst)